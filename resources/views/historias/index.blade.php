...
@section('content_header')
    <h1>Lista Historias Clínicas</h1>
@stop
@section('css')
            ...
            @can('Admin')
                <tr>
                    ...
                    @foreach ($pacientes as $paciente)
                        @if($historia->paciente_id == $paciente->id)
                    ...
                    <td>
                        ...
                        <form action="{{route('historias.destroy',$historia)}}" method="POST">
                            ...
                            <button class="btn btn-danger btn-sm text-light rounded-pill" style="margin-top: 0.35rem" onclick="return confirm('¿ESTÁ SEGURO DE BORRAR?')" value="Borrar"><i class="fas fa-trash"></i> Eliminar</button>
                           
                        </form>

                        <a class="btn btn-warning btn-sm text-light rounded-pill" style="margin-top: 5px" href="{{route('historias.show', $historia)}}"><i class="fas fa-eye"></i></i>  Ver </a> 
                         
                        <a class="btn btn-primary btn-sm text-light rounded-pill" style="margin-top: 5px" href="{{route('historias.edit', $historia->id)}}"><i class="fas fa-pencil-alt"></i>  Editar</a>  
